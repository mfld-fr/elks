...
{
    debugmem("zero_read()\n");
    fmemsetb((word_t) data, current->mm.seg_data->base, 0, (word_t) len);
    filp->f_pos += len;
    return (size_t)len;
}
	      ...
	      char *data, size_t len)
{
    ...
    fmemcpyb(data, current->mm.seg_code->base, soff, sseg, (word_t) len);
    filp->f_pos += len;
    return (size_t) len;
}
	       ...
	       char *data, size_t len)
{
    ...
    fmemcpyb(doff, dseg, (word_t) data, current->mm.seg_data->base, (word_t) len);
    filp->f_pos += len;
    return len;
}
